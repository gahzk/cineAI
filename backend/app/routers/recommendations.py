...
@router.get("/trending")
async def trending_this_week(
    _: User = Depends(get_current_user),
):
    """Return trending movies and TV shows of the week from TMDB."""
    g = tmdb.get_genres()
    results = []
    for kind in ("movie", "tv"):
        data = tmdb.tmdb_request(f"/trending/{kind}/week", {"language": "pt-BR"})
        g_map = g["movie_genres"] if kind == "movie" else g["tv_genres"]
        for it in data.get("results", [])[:10]:
            item = tmdb.build_catalog_item(it, kind, g_map)
            if item:
                item["poster_url"] = tmdb.poster_url(item.get("poster_path"))
                item["synopsis"] = it.get("overview", "")
                results.append(item)
