...
# Garante que 'requests', 'rapidfuzz' e 'rich' estejam instalados
...
# Chave Bearer da API TMDB (Substitua se necessário)
...
def _norm(s: str) -> str:
    ...
    if s is None: return ""
    # "NFKD" decompõe caracteres (ex: 'ç' -> 'c' + '̧')
    # O loop então mantém apenas os caracteres base (não combinados/acentos)
...
def _tmdb_request(path, params=None, retries=3):
    """Wrapper de requisição para a API TMDB com lógica de retry e rate limiting (429)."""
    wait_time = 0.25  # Tempo de espera inicial
    for _ in range(retries + 1):
        try:
            r = SESSION.get(f"{BASE_URL}{path}", params=params or {}, timeout=20);
            # Se recebermos 'Too Many Requests', esperamos o tempo sugerido ou aumentamos o backoff
            if r.status_code == 429:
                time.sleep(float(r.headers.get("Retry-After", wait_time)));
                wait_time = min(3.0, wait_time * 1.8); # Backoff exponencial com teto
                continue
            r.raise_for_status(); # Levanta erro para outros status 4xx/5xx
            return r.json()
        except requests.RequestException:
            # Em caso de falha de conexão, espera e tenta novamente
            time.sleep(wait_time);
            wait_time = min(3.0, wait_time * 1.8)
    return {} # Retorna vazio se todas as tentativas falharem

def _get_year(date_str):
    """Extrai o ano de uma string de data (ex: "2023-10-17")."""
    if not date_str: return None
    try: return int(date_str.split("-")[0])
    except (ValueError, IndexError): return None

def _is_cache_valid(filepath, days=CACHE_EXPIRATION_DAYS):
    """Verifica se um arquivo de cache existe e ainda é válido (não expirou)."""
    if not os.path.exists(filepath): return False
    # Compara o tempo de modificação do arquivo com o tempo atual
    return (time.time() - os.path.getmtime(filepath)) < (days * 86400) # 86400 segundos = 1 dia

_genres_cache = None
def _get_genres_cached():
    """Obtém o mapa de gêneros (ID -> Nome) da API ou do cache."""
    global _genres_cache
    if _genres_cache: return _genres_cache # Cache em memória
...
def _discover_page(task):
    """Função worker: Busca uma única página de resultados do 'discover' da API."""
    kind, g, s, d, p, lang, all_g, keywords = task; d0, d1 = d;
    params={"language":lang,"sort_by":s,"page":p,"include_adult":"false","vote_count.gte":100} # Filtro base
    if keywords: params["with_keywords"] = keywords # Adiciona palavras-chave (ex: anime)

    data = _tmdb_request(f"/discover/{kind}", params)

    # Mapeia os IDs de gênero para nomes
    g_map = all_g["movie_genres"] if kind == "movie" else all_g["tv_genres"]
    # Define as chaves corretas (filmes e séries usam nomes de campos diferentes)
    ...
    for it in data.get("results",[]):
        ...
        # Formata o item para o nosso catálogo local
    ...
    # Retorna os resultados e um booleano (se existem mais páginas)
    return res, (data.get("page",0) < data.get("total_pages",0))

def _fetch_live_catalog(target, lang, workers):
    """Busca intensiva na API para montar um catálogo local de 'target' títulos."""
    ...
    # --- Tarefas Prioritárias (Animes e Séries Populares) ---
    anime_keyword = "210024" # Keyword ID para "Anime" no TMDB
    anime_genres = [t_ids_map.get("Animação"), t_ids_map.get("Ação e Aventura"), t_ids_map.get("Sci-Fi & Fantasy")]
    anime_genres_clean = [gid for gid in anime_genres if gid] # Remove Nones se o gênero não for encontrado
    if anime_genres_clean:
        for sort_key in ["popularity.desc", "vote_average.desc"]:
            for page in range(1, 11): # Busca 10 páginas de animes
                series_priority_tasks.append(("tv", anime_genres_clean, sort_key, ("2000-01-01", "2025-12-31"), page, lang, g, anime_keyword))

    # Prioriza séries recentes e populares/bem avaliadas
    for sort_key in ["popularity.desc", "vote_average.desc"]:
        for genre_chunk in [t_ids[i:i+2] for i in range(0,len(t_ids),2)]: # Agrupa gêneros de 2 em 2
            for page in range(1, 4): # Busca 3 páginas por chunk
                series_priority_tasks.append(("tv", genre_chunk, sort_key, ("2010-01-01", "2025-12-31"), page, lang, g, None))

    # --- Tarefas Gerais (Filmes e Séries de várias décadas) ---
    general_tasks = []
    ...
    for d in decades:
        for s in sorts:
            ...
            # Adiciona tarefas de filmes (1 página por chunk de gênero)
    ...
    with Progress(SpinnerColumn(style=THEME["primary_fg"]),TextColumn("[progress.description]{task.description}"),BarColumn(bar_width=None),TextColumn("[progress.percentage]{task.percentage:>3.0f}%"),TextColumn("• {task.completed}/{task.total} títulos"),TimeElapsedColumn(),console=console,transient=True) as prog:
        ...
        with cf.ThreadPoolExecutor(max_workers=workers) as ex:
            # Submete um lote inicial de tarefas
            ...
            while futures and len(catalog)<target:
                # Espera a primeira tarefa terminar
                done,_=cf.wait(futures,return_when=cf.FIRST_COMPLETED)

                for fut in done:
                    orig_task=futures.pop(fut) # Remove a tarefa concluída
                    try:
                        batch,has_more=fut.result(); added=0
                        for item in batch:
                            key=(item["type"],item["id"]);
                            if key not in seen_ids: # Evita duplicatas
                                seen_ids.add(key); catalog.append(item); added+=1

                        prog.update(task_id,advance=added) # Atualiza a barra de progresso

                        # Lógica de Paginação: se a tarefa retornou resultados e há mais páginas,
                        # adiciona a próxima página na fila de tarefas.
                        (kind,gn,s,d,p,ln,gs,kw)=orig_task
                        if has_more and added > 0 and p < 15 and len(catalog) < target: # Limita a 15 páginas por busca
                            tasks.append((kind,gn,s,d,p+1,ln,gs,kw))
                    except Exception: pass # Ignora falhas em tarefas individuais

                    # Adiciona novas tarefas à fila se houver espaço e tarefas
                    if tasks and len(futures)<workers*2:
                        futures[ex.submit(_discover_page,tasks.popleft())]=tasks[0]

    return catalog[:target] # Retorna o catálogo no tamanho desejado

def build_catalog_cached(target, force_rebuild):
    """Ponto de entrada: decide se busca o catálogo do cache ou da API."""
...
# Mapeamento de "temas" para gêneros
...
def map_terms_to_genres(terms):
    """Converte termos de busca do usuário (ex: 'suspense', 'espacial') em gêneros canônicos."""
    ...
    for term in terms:
        ...
        # 1. Verifica se o termo é um "tema" (ex: "espacial")
        for theme_key, theme_genres in THEMES.items():
            if fuzz.partial_ratio(norm_term,theme_key)>85: out.update(theme_genres)

        # 2. Verifica o match mais próximo com um gênero canônico
        # 'token_set_ratio' é bom para ordens diferentes ("Ficção Científica" vs "Científica Ficção")
        best=process.extractOne(norm_term,norm_canon,scorer=fuzz.token_set_ratio)
        if best and best[1]>75: # Se o match for > 75%
            out.add(GEN_CANON[best[2]])

    return list(out)

def get_duration_score(pref, runtime):
    """Calcula um score de 0.0 a 1.0 para a duração, baseado na preferência."""
    if runtime is None: return 0.5 # Neutro se não soubermos a duração
    t=_norm(pref or "");

    # Retorna score 1.0 se for exatamente o ideal, decaindo linearmente
    if "curt" in t: return max(0.0, 1 - abs(runtime - 90) / 90)  # Perto de 90 min
    if "medi" in t: return max(0.0, 1 - abs(runtime - 120) / 60) # Perto de 120 min
    if "long" in t: return max(0.0, 1 - abs(runtime - 180) / 180) # Perto de 180 min

    return 0.5 # Neutro se não houver preferência

TYPE_MATCH_STRONG_BONUS = 60 # Bônus alto se acertar o tipo (filme/série)
def score_item(item, prefs):
    """O cérebro do CineAI: calcula a pontuação de um item baseado nas preferências."""
    s=0.0; i_g_norm=_norm(item["genres"]) # Gêneros normalizados do item

    # 1. Bônus por Tipo (Filme/Série)
    if prefs["type"] and prefs["type"] == item["type"]:
        s += TYPE_MATCH_STRONG_BONUS

    # 2. Bônus/Penalidade por Gênero
    for g_norm in prefs["inc_norm"]: # Gêneros incluídos
        if g_norm in i_g_norm: s+=15
    for g_norm in prefs["exc_norm"]: # Gêneros excluídos
        if g_norm in i_g_norm: s-=25

    # 3. Score de Duração
    s+=10 * get_duration_score(prefs["dur"], item.get("runtime"))

    # 4. Score de Qualidade (Nota) e Popularidade
    # 'q' (qualidade): Normaliza a nota e pondera pela contagem de votos (até 5000)
    q = (item["vote_avg"] / 10.0) * (min(1.0, item["vote_cnt"] / 5000.0));
    # 'p' (popularidade): Normaliza a popularidade (até 1000)
    p = min(1.0, item["popularity"] / 1000.0)

    s += prefs["w_rating"] * q * 12 # Aplica pesos da preferência do usuário
    s += prefs["w_pop"] * p * 12

    # 5. Score de Ano (Clássico vs Novo)
    y = item.get("year") or 2000
    if "classic_focus" in prefs and prefs["classic_focus"]:
        s += max(0, (2000 - y) / 10) # Bonifica filmes mais antigos (pré-2000)
    elif prefs["prefer_new"]:
        s += max(0, (y - 2000) / 3)  # Bonifica filmes pós-2000
    else:
        s -= max(0, (y - 2010) / 4)  # Penaliza levemente filmes muito novos se a preferência for neutra

    return s

def _fetch_details_concurrent(items_to_detail: List[Dict]) -> List[Dict]:
    """Busca detalhes (sinopse, elenco, etc.) para os N melhores itens, em paralelo."""
    def fetch_one(item):
        # Busca detalhes E provedores (watch/providers)
        path = f"/{item['type']}/{item['id']}"
        data = _tmdb_request(path, params={"language": "pt-BR", "append_to_response": "credits,watch/providers"})
        ...
        # --- CORREÇÃO DO BUG "N/A" ---
        # Agora, se não encontrarmos, salvamos "N/A" como texto puro.

        if item["type"] == "movie":
            directors = [c["name"] for c in data.get("credits", {}).get("crew", []) if c.get("job") == "Director"]
            item["director"] = ", ".join(directors) if directors else "N/A"
        else:
            creators = [c["name"] for c in data.get("created_by", [])]
            item["director"] = ", ".join(creators) if creators else "N/A"

        cast = [c["name"] for c in data.get("credits", {}).get("cast", [])[:4]]
        item["cast"] = ", ".join(cast) if cast else "N/A"

        # Lógica de Provedores (Onde Assistir)
        providers_data = data.get("watch/providers", {}).get("results", {}).get("BR", {})
        streaming_providers = [p["provider_name"] for p in providers_data.get("flatrate", [])] # "flatrate" = Assinatura

        if streaming_providers:
            unique_providers = list(dict.fromkeys(streaming_providers)) # Remove duplicatas
            item["providers"] = ", ".join(unique_providers)
        else:
            item["providers"] = "Não encontrado em streaming (assinatura)"
        # --- Fim da Correção ---

        return item

    # Executa as buscas de detalhes em 4 threads
...
def banner():
    """Imprime o banner inicial do CineAI."""
...
def get_match_color(score_pct: float) -> Style:
    """Retorna um estilo de cor baseado na pontuação (verde alto, laranja baixo)."""
    if score_pct > 92: return Style(color="#00FF7F") # Verde Brilhante
    if score_pct > 80: return Style(color="#ADFF2F") # Verde-Limão
    if score_pct > 65: return Style(color=THEME["warning"]) # Amarelo
    return Style(color="#FFA07A") # Salmão (Laranja claro)

def show_results(scored_items: List[tuple], top_n=3):
    """Exibe os resultados formatados em tabelas e painéis do 'rich'."""
    if not scored_items: return

    # Normaliza os scores (de 40% a 99%) para exibição
    ...
    for score, item in scored_items[:top_n]:
        # Converte o score bruto para uma porcentagem (40-99%)
        ...
        nota_style = get_match_color(item['vote_avg'] * 10) # Usa a mesma escala de cor para a nota
    ...
    for i, (_, item) in enumerate(scored_items[:top_n], 1):
        ...
        # Garante que a sinopse tenha um estilo 'dim' se for "não disponível"
        synopsis_str = item.get('synopsis', 'Sinopse não disponível.')
        synopsis_style = THEME['dim'] if "não disponível" in synopsis_str else THEME['text']
        synopsis = Text(f"\n  {synopsis_str}\n", style=synopsis_style, justify="full")

        # Grid para Diretor/Elenco
        ...
        # --- CORREÇÃO DO BUG "N/A" ---
        # Pega os valores de texto puro
        providers_str = item.get('providers', 'N/A')
        director_str = item.get('director', 'N/A')
        cast_str = item.get('cast', 'N/A')

        # Define o estilo com base no conteúdo (cinza se for 'N/A' ou 'Não encontrado')
        providers_style = THEME['text'] if "Não encontrado" not in providers_str else THEME['dim']
        director_style = THEME['text'] if director_str != 'N/A' else THEME['dim']
        cast_style = THEME['text'] if cast_str != 'N/A' else THEME['dim']

        # Adiciona as linhas com os estilos corretos
        details_grid.add_row(f"[bold {THEME['prompt']}]Onde Ver (BR):[/]", Text(providers_str, style=providers_style))
        details_grid.add_row(f"[bold {THEME['prompt']}]{director_label}[/]", Text(director_str, style=director_style))
        details_grid.add_row(f"[bold {THEME['prompt']}]Elenco:[/]", Text(cast_str, style=cast_style))
        # --- Fim da Correção ---

        # Comentário da IA
        ai_comment = Text.from_markup(f"\n[bold {THEME['primary_fg']}]💬 Comentário do CineAI:[/]\n[i]{item.get('ai_comment', '...')}[/i]")

        # Agrupa tudo em um 'Group' para renderização coesa
...
def _create_prompt_grid(prompt_renderable, explanation_text):
    """Helper de UI: cria um grid para alinhar a pergunta (esquerda) e a explicação (direita)."""
...
def fuzzy_prompt(prompt_text, choices, default=None, threshold=70, explanation=""):
    """Um prompt que aceita respostas "fuzzy" (ex: 'com' -> 'Comédia')."""
    ...
    while True:
        ...
        # Encontra o melhor match entre a entrada e as escolhas
        best=process.extractOne(norm_input,norm_choices,scorer=fuzz.token_set_ratio)

        if best and best[1]>=threshold:
            return choices[best[2]] # Retorna a escolha original (ex: "Comédia")
        else:
            console.print(f"  [{THEME['error']}]Opção '[white]{user_input}[/white]' não reconhecida.[/{THEME['error']}]")

def ask_preferences_panel():
    """Exibe o formulário de perguntas principal para o usuário."""
    ...
    # Mapas para converter as respostas do usuário em valores da API/lógica
    tipo_map={"Filme":"movie","Série":"tv","Tanto faz":""};
    foco_map={"Nota":(1.0,0.4),"Popularidade":(0.4,1.0),"Equilíbrio":(0.8,0.8)} # (w_rating, w_pop)

    # --- Perguntas Refinadas ---

    tipo = fuzzy_prompt("Qual o formato?",
                        choices=list(tipo_map.keys()),
                        default="Tanto faz",
                        explanation="Filme, Série ou Tanto faz? Default: 'Tanto faz'")

    foco = fuzzy_prompt("Qual o seu foco?",
                        choices=list(foco_map.keys()),
                        default="Equilíbrio",
                        explanation="Nota (crítica), Popularidade (fama) ou Equilíbrio? Default: 'Equilíbrio'")

    _create_prompt_grid(Text.from_markup(f"[bold {THEME['prompt']}]❯ Quais gêneros ou temas você curte?[/]"),
                        "Ex: Ação, comédia, espacial, suspense. (Separe por vírgulas)")
    inc_raw=Prompt.ask("  ╰─> ",default="")

    _create_prompt_grid(Text.from_markup(f"[bold {THEME['prompt']}]❯ Algum gênero ou tema para evitar?[/]"),
                        "Ex: Terror, musical, etc. (Separe por vírgulas)")
    exc_raw=Prompt.ask("  ╰─> ",default="")

    dur = fuzzy_prompt("Qual a duração ideal?",
                       choices=["Curta", "Média", "Longa", "Qualquer"],
                       default="Qualquer",
                       explanation="Curta (~90m), Média (~120m), Longa (150m+). Default: 'Qualquer'")

    # Pergunta de 'sim/nao'
    prefer_new = Confirm.ask(Text.from_markup(f"[bold {THEME['prompt']}]❯ Dar preferência a títulos recentes?[/]"),
                             choices=["sim", "nao"],
                             default=False)
    ...
    # Retorna o dicionário de preferências (prefs)
...
def refine_preferences(prefs):
    """Permite ao usuário fazer pequenos ajustes nas preferências sem recomeçar."""
    ...
    # Atualiza o dicionário 'prefs' baseado na escolha
    if "clássicos" in action:
        prefs["classic_focus"]=True; prefs["prefer_new"]=False; console.print(f"[bold {THEME['success']}]✓[/] Foco ajustado para clássicos.")
    elif "aclamados" in action:
        prefs["w_rating"]=1.2; prefs["w_pop"]=0.3; console.print(f"[bold {THEME['success']}]✓[/] Foco ajustado para filmes aclamados.")
    elif "cult" in action:
        prefs["w_rating"]=0.6; prefs["w_pop"]=0.1; console.print(f"[bold {THEME['success']}]✓[/] Foco ajustado para títulos menos populares.")
        ...
        if g_to_add:
            prefs["inc"]=list(set(prefs["inc"]+g_to_add)); prefs["inc_norm"]=[_norm(g) for g in prefs["inc"]];
            console.print(f"[bold {THEME['success']}]✓[/] Adicionado: {g_to_add}")
        ...
        if g_to_rem:
            ...
            prefs["exc"]=list(set(prefs["exc"]+g_to_rem)); prefs["exc_norm"]=[_norm(g) for g in prefs["exc"]];
            console.print(f"[bold {THEME['success']}]✓[/] Removido/Evitado: {g_to_rem}")
...
def cli(args):
    ...
    # Etapa 1: Construir/Carregar o catálogo
    catalog = build_catalog_cached(args.target, args.rebuild)
    if not catalog:
        console.print(Panel(f"[{THEME['error']}]Falha ao construir o catálogo.[/]", box=MINIMAL, border_style=THEME['error'])); return

    console.print(Rule(style=THEME["dim"]))

    # Etapa 2: Perguntar as preferências iniciais
    prefs = ask_preferences_panel()

    # Loop principal da aplicação
    while True:
        # Etapa 3: Pontuar e Ordenar
        ...
        # Etapa 4: Buscar detalhes apenas para os N melhores (para economizar API)
        ...
        if items_needing_details:
            ...
            # Gera o comentário de IA após ter os detalhes
        ...
        elif action == "Nova Busca Completa":
            console.print(Rule(style=THEME["dim"])); prefs = ask_preferences_panel()
        elif action == "Refinar Sugestões":
            console.print(Rule(style=THEME["dim"])); prefs = refine_preferences(prefs)
...
if __name__ == "__main__":
    ...
    try:
        # Tenta parsear os argumentos (funciona em terminal)
        args = parser.parse_args()
    except SystemExit:
        # Se falhar (ex: no Colab/Jupyter), usa os defaults
        args = parser.parse_args([])

    try:
        # Validação simples da chave de API
        if not TMDB_BEARER or "eyJ" not in TMDB_BEARER:
            ...
            cli(args) # Inicia a aplicação
    except KeyboardInterrupt:
        console.print(f"\n[{THEME['error']}]Encerrado pelo usuário.[/{THEME['error']}]")
    except Exception as e:
        # Imprime o traceback completo da exceção se algo der errado
